...
<script runat="server">
	// WET4 Version 
	//
	// datatable implementation for WET4, using the datatable.net library
	// note: datatable implementation uses the WET systax.
	// https://datatables.net/manual/index
	//
	// The library is already provided via 
	// https://www.canada.ca/etc/designs/canada/cdts/gcweb/v4_0_32/cdts/compiled/soyutils.js
	// https://www.canada.ca/etc/designs/canada/cdts/gcweb/v4_0_32/cdts/compiled/wet-[lang].js
	...
	private JObject getRenderMode(JObject jOptions) {
		...
		if (renderMode != null && renderMode.Equals("true")) {
			...
			string basePath = getBasePath(); //(!Context.Items["basePath"].Equals("")) ? basePath = (string)Context.Items["basePath"] : "";
							...
							<apn:ChooseControl runat="server">
								<apn:WhenControl type="ROW" runat="server">
									...
									<apn:forEach runat="server" id="trFieldRow">
										<apn:ChooseControl runat="server">
											<apn:WhenControl type="GROUP" runat="server">
												<td class='<apn:cssClass runat="server" />' style='<apn:cssStyle runat="server" />'><% Server.Execute(resolvePath("/controls/custom/span.aspx")); %></td>
											</apn:WhenControl>
